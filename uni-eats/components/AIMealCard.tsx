...
export const AIMealCard = ({ meal, canteenName, reason }: AIMealCardProps) => {
  ...
  const priceDisplay = selectedPrice.price !== null
    ? t('common.priceFormat', { value: selectedPrice.price.toFixed(2) })
    : '';

  const handlePress = () => {
    // Navigate to meal detail with serialized params (as expected by meal-detail.tsx)
    router.push({
...
const styles = StyleSheet.create({
  wrapper: {
    marginBottom: 16,
    marginLeft: 16, // Indent slightly to align with AI messages or look grouped
    marginRight: 32,
  },
  reasonContainer: {
    flexDirection: 'row',
    ...
    marginBottom: -4, // Overlap slightly or connect visually
    zIndex: 1,
  ...
  reasonText: {
    fontSize: 13,
    fontFamily: Fonts.regular, // Using regular font from theme
    lineHeight: 18,
    flex: 1,
  },
  cardContainer: {
    ...
    alignItems: 'center',
    borderRadius: 16,
    borderTopLeftRadius: 4, // Visual connection to reason
    padding: 8,
    borderWidth: 1,
    gap: 12,
