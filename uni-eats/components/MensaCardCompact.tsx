...
export function MensaCardCompact({
  ...
  const resolvedDistance = distance ?? t('common.distanceUnavailable');
  const resolvedTravelTime = travelTime ?? t('common.timeUnavailable');
  const rating = canteen.rating ? canteen.rating.toFixed(1) : t('common.ratingUnavailable');
